...
#     def __str__(self):
#         return "%s - %s - %s - %s" % (self.first_name, self.username, self.email, self.last_login)
...
class Competenciasusuario(models.Model):
    ...
    def __str__(self):
        return "%s - %s - %s - %s" % (self.idregistrocompusuario, self.fkcompetence, self.fkuser, self.tiene)
